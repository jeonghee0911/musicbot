...
class MusicManager {
    ...
    async handleVideo(message, voiceChannel, song) {
        ...
        if (!serverQueue) {
            ...
            try {
                ...
                console.error(`오류가 발생했어요 ㅠㅠ..: ${error}`);
                this.queue.delete(message.guild.id);
                this.manager.leave(message.guild.id);
                message.channel.send(`오류가 발생했어요 ㅠㅠ.. 오류가 계속된다면 정희인데 _#0911 님에게 문의 주세요! 에러내용: ${error.message}`);
            }
        } else {
            serverQueue.songs.push(song);
            message.channel.send(`성공! 재생목록에 **${song.info.title}** 를 추가했어요~!`);
        }
    }
    ...
    play(guild, song) {
        ...
        if (!song) {
            serverQueue.textChannel.send("재생목록의 음악이 다 끝났어요! 전 이제 나가볼게요...");
            ...
            serverQueue.textChannel.send(`지금 재생 중: **${song.info.title}** by *${song.info.author}*`);
        }
    }
